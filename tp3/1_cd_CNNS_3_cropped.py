...
def main(batch_size=128, lr=0.1, epochs=5, cuda=False):
    ...
    # TensorBoard writer
    writer = SummaryWriter(log_dir=f"runs/batch{batch_size}_lr{lr}_lr")

    for epoch_idx in range(epochs):
        print(f"\n===== EPOCH {epoch_idx + 1}/{epochs} =====")

        # Train phase
        train_top1, train_top5, train_loss = epoch(train_loader, model, criterion, optimizer, cuda,epoch_idx=epoch_idx)
