...
def epoch(data, model, criterion,writer, optimizer=None, cuda=False,epoch_idx=0):
    """
...
def main(batch_size=128, lr=0.1, epochs=5, cuda=False):
    ...
    # TensorBoard writer
    writer = SummaryWriter(log_dir=f"runs/batch{batch_size}_lr{lr}_normalized")

    for epoch_idx in range(epochs):
        print(f"\n===== EPOCH {epoch_idx + 1}/{epochs} =====")

        # Train phase
        train_top1, train_top5, train_loss = epoch(train_loader, model,writer, criterion, optimizer, cuda,epoch_idx=epoch_idx)

        # Eval phase
        test_top1, test_top5, test_loss = epoch(test_loader, model, writer, criterion, cuda=cuda,epoch_idx=epoch_idx)
...
if __name__ == "__main__":
    for batch_size in [64, 128, 256]:
        for lr in [1e-3, 1e-2, 1e-1]:
            main(batch_size=batch_size, lr=lr, cuda=True)
